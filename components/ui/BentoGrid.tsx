...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid grid-cols-1 lg:grid-cols-5 md:grid-cols-3 gap-4 max-w-7xl mx-auto ",
        className
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    navigator.clipboard.writeText("contact@milankumar.com");
    setCopied(true);
  };
  ...
  return (
    <div
      className={cn(
        "row-span-1 rounded-3xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none bg-white relative overflow-hidden border border-white/[0.1]",
        className
      )}
      style={{
          ...
          {id === 1 && (
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:right-9">
              <div className="flex flex-col gap-3 lg:gap-8">
                {["React.js", "Next.js", "TypeScript", "Nodejs"]?.map(
                  (item) => (
                    <span
                      key={item}
